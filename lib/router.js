...
function Router(req, res, context) {
  this.req = req;
  this.res = res;
...
Router.prototype.route = function(callback) {
  ...
  //Route '/' to home
  if(reqURL === '/' || reqURL === '') {
    reqURL = '/' + this.context.config.get('home');
  }
...
Router.prototype._applyRouteIfExists = function(methodName) {
  ...
  if(fn !== false) {
    ...
    var controllerScope = new this.context.Controller(this.req, this.res, url);
...
Router.prototype._getMethod = function(url, method) {
  var cntrl = this.context.controllers.get(url);
